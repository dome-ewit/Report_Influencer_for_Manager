...
class insert_update_over_all extends Model
{
    ...
    public function select_insert_update_over_all($data_update_insert)
    {
        $db = new insert_update_over_all();
        $select_data_table_overall = $db->query("SELECT network,total_post,unique_post
                                                FROM over_all
                                                WHERE  network = '{$data_update_insert['network']}'");

        $select_data_overall = $select_data_table_overall->getResultArray();
        if ($select_data_overall == null) {
            ...
            $over_all = new insert_update_over_all();
            $update_update = $over_all->update_total_unique_post_over_all($data_update_insert['network'], $select_data_overall[0]['total_post'], $select_data_overall[0]['unique_post'], $data_update_insert['count_unique_post'], $data_update_insert['count_total_post']);
        }
        return $select_data_overall;
    }
